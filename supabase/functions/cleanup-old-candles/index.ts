...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!config?.is_enabled) {
      console.log('Cleanup old candles job is disabled, skipping execution')
      return new Response(
    ...
    const retentionDays = config?.retention_days || 14
    ...
    while (batchCount < MAX_BATCHES) {
      const { data, error } = await supabase.rpc('cleanup_old_candles_batch', {
      ...
      console.log(`Batch ${batchCount}: Deleted ${deletedCount} candles (total: ${totalDeleted})`)
    ...
    // Get remaining count of old candles
    let remainingCount = 0
    if (hitLimit || totalDeleted > 0) {
      const { count, error: countError } = await supabase
        .from('candles_1min')
        .select('*', { count: 'exact', head: true })
        .lt('bucket', cutoffDate.toISOString())
    ...
    console.log(`Cleanup complete. Deleted ${totalDeleted} candles in ${batchCount} batches. Remaining: ${remainingCount}`)
    ...
    const message = hitLimit
      ...
      : totalDeleted > 0
        ? `Cleanup complete. Deleted ${totalDeleted.toLocaleString()} candles.`
        : 'No old candles to clean up.'
